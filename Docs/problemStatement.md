...
We are building an automated weekly “pulse” that turns public Google Play reviews for the Groww app into a one-page insight report and delivers it to stakeholders through Google Workspace, using MCP (Model Context Protocol) so that writes to Google Docs and Gmail go through dedicated MCP servers that we will create and provide in this project.

**Supported product (initial):** Groww.
...
- **Ingest public reviews** from the last 8–12 weeks (configurable window) from Google Play (scraper-based), for the Groww app.
...
- **Deliver outputs only through custom Google Workspace MCP servers provided in this project**:
...
| :--- | :--- |
| **Data retrieval** | Ingestion modules (Google Play Store only) |
...
| **Human-visible delivery** | MCP tools only → Custom Google Docs MCP + Gmail MCP servers built in this project |

*Note: The agent is an MCP host/client; it does not embed Google credentials or call the Docs/Gmail REST APIs directly for delivery. The MCP servers will be developed as part of this project.*
...
- Social sources (Twitter, Reddit, etc.) or App Store reviews in the initial scope.
- Storing Google OAuth secrets in the agent codebase—they belong in the custom MCP servers’ configuration.
